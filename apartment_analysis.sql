...
-- **** ANALYSIS OF AUSTIN **** --
	# Monthly & Yearly averages based over 2022 - 2024 years to capture seasonal trends. 
...
WHERE location_name = 'Austin, TX'
    AND `date` BETWEEN '2022-01-1' AND '2024-12-01'
GROUP BY month_name, month_num
ORDER BY month_num; #ORDER BY avg_vacancy_index DESC;

-- 2. Austin > Average Vacancy Index by Year in past 2 years
SELECT 
    YEAR(`date`) AS year, 
    ROUND(AVG(vacancy_index) * 100, 2) AS avg_vacancy_index
FROM apartment_vacancy_staging
...
-- 4. Austin > Average Time On Market by Year in past 2 years
SELECT
    YEAR(`date`) AS year, 
    ROUND(AVG(days_on_market), 2) AS avg_days
FROM apartment_tom_staging
...
-- 6. Austin > Average Rent Estimates by Year in past 2 years
SELECT
    YEAR(`date`) AS year, 
    ROUND(AVG(median_rent), 2) AS avg_monthly_median_rent
FROM apartment_rent_estimates_staging
...
WHERE v.location_name = 'Austin, TX'
    AND v.`date` BETWEEN '2022-01-1' AND '2024-12-01'
...
-- 2. United States > Average Vacancy Index by Year in past 2 years
SELECT
    YEAR(`date`) AS year,
    ROUND(AVG(vacancy_index) * 100, 2) AS avg_vacancy_index
FROM apartment_vacancy_staging
...
-- 4. United States > Average Time On Market by Year in past 2 years
SELECT
    YEAR(`date`) AS year,
    ROUND(AVG(days_on_market), 2) AS avg_days
FROM apartment_tom_staging
...
-- 6. United States > Average Rent Estimates by Year in past 2 years
SELECT
    YEAR(`date`) AS year,
    ROUND(AVG(median_rent), 2) AS avg_monthly_median_rent
FROM apartment_rent_estimates_staging
